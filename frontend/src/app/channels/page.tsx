      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filtered.map((ch) => {
          ...
          return (
            <div
              key={ch.id}
              className="bg-white/3 border border-white/10 rounded-2xl p-4 backdrop-blur-xl hover:border-white/20 transition-all"
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-white/10 text-white/40 uppercase tracking-wider">
                      {ch.category}
                    </span>
                  </div>
                </div>
              ...
              {ch.connected ? (
                <div className="flex gap-2">
                  <span className="flex-1 text-center text-xs py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                    Connected
                  </span>
                  <button
                    onClick={() => handleDisconnect(ch.id)}
                    className="px-3 py-1.5 rounded-lg text-xs text-red-400 bg-red-500/10 border border-red-500/20 hover:bg-red-500/20"
                  >
                    Disconnect
                  </button>
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs text-cyan-400 bg-cyan-500/10 border border-cyan-500/20 hover:bg-cyan-500/20 transition-all"
                >
                  {isExpanded ? (
                ...
                <div className="mt-3 pt-3 border-t border-white/10 space-y-2">
                  {ch.fields.map((f) => (
                    <div key={f.key}>
                      ...
                      <input
                        ...
                        className="w-full mt-0.5 px-2.5 py-1.5 bg-white/5 border border-white/10 rounded-lg text-xs text-white placeholder:text-white/20 focus:outline-none focus:border-cyan-500/50"
                      />
                    </div>
                  ))}
                  <button
                    ...
                    className="w-full mt-2 py-2 rounded-lg text-xs font-medium bg-cyan-500/20 text-cyan-400 border border-cyan-500/30 hover:bg-cyan-500/30 disabled:opacity-50 transition-all"
                  >
                    {connecting === ch.id ? (
                  ...
                  {ch.docs && (
                    <a
                      ...
                      className="block text-center text-[10px] text-white/30 hover:text-cyan-400"
                    >
                      View Documentation →
                    </a>
                  )}
